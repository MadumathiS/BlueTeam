...
exploitation activity consistent with a Red Team engagement. The activity
followed a recognizable chain: automated port/path scanning, discovery of
`robots.txt`, triggering of the honeypot decoy, and enumeration of the IDOR
endpoint, with repeated access to the debug endpoint. Every stage was detected
through layered logging; the honeypot produced three CRITICAL alerts
automatically. No production data was exposed — the only "sensitive" material
reached was the intentional decoy (a fake SQL dump).

---

## 2. Attribution note

Most malicious requests on July 24 originated from `172.18.0.1`, the Docker
bridge gateway (host-originated traffic reaching the container), with additional
requests from `192.168.20.12` (the lab VM's LAN address). Traffic from the LAN
address represents scanning from the network; host-originated tooling appears as
the gateway address. "The attacker" below refers to the source(s) generating this
activity, regardless of network path.
...
| 14:57:09 | 172.18.0.1 | ~15 paths in one second (`/.git/HEAD`, `/nmaplowercheck…`, `/HNAP1`, etc.) | Automated scan | access.log |
...
**Wireshark evidence.** Packet capture of Red Team activity was taken on the lab
host (single-host setup, loopback interface). The scan capture shows a single
source issuing SYN packets to hundreds of ports within milliseconds, and on open
ports completing the handshake then immediately resetting with no data. A
contrasting capture of legitimate traffic shows a full handshake, a real HTTP GET
and response, and a graceful close. (See annotated screenshots:
`wireshark-scan-portsweep`, `wireshark-openport-detection`,
`wireshark-legitimate-traffic`.)
...
`/api/v1/users/1/setup-status` and `/users/2/setup-status` directly — classic
...
hardcoded bearer token in `/api/profile` (`valid-session-token-123`). This was
not one of the three intentional vulnerabilities and is treated as an additional
finding, remediated in the patched build.
...
**Triage lesson.** The honeypot hit is unambiguous signal; the scan burst is
signal by pattern; ordinary favicon/registration traffic is benign noise.
Correlating a single source across multiple detections is what elevates an event
from "suspicious" to "confirmed incident."

---

## 7. Recommended production fixes

**Vulnerability 1 — `/api/debug`.** Remove the endpoint or gate it behind
authentication and restrict to internal networks; never expose interpreter or
config details. Disable Flask debug mode.

**Vulnerability 2 — TOTP replay.** Record each consumed code/counter per user and
reject reuse within the validity window; keep the acceptance window minimal.
...
expose internal APIs to untrusted networks. (Patched build adds a required
shared-secret control as a first step.)
...
**Cross-cutting.** Production WSGI server, HTTPS, secrets via a secret manager,
and authentication on the ELK stack.
...
high-confidence alert. All findings — the three intentional vulnerabilities and
the additional hardcoded-token finding — are remediated in the patched build.
